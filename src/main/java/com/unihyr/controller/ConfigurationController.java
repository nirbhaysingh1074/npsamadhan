...
public class ConfigurationController
{
	...
	public String adminEditConfiguration(ModelMap map)
	{
		//map.addAttribute("configVariables",configurationService.getConfigVariab);
	...
	public String addConfiguration(Principal principal, ModelMap map
		...
		if(result.hasErrors() || configVariable.getVarName().trim().length() <1||!configurationService.getConfigVariable(configVariable.getVarName().trim()).isEmpty())
		{
			map.addAttribute("var_error", "Please enter Variable name");
			if(!configurationService.getConfigVariable(configVariable.getVarName().trim()).isEmpty())
